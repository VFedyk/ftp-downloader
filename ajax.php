...
set_time_limit(0);
define('DOWNLOAD_STATE_FILENAME', 'state.json');
...
function downloadCallback($downloaded, $allSize, $params)
{
    ...
    file_put_contents(DOWNLOAD_STATE_FILENAME, json_encode($params));
}
...
if (isset($_GET['operation']) && $_GET['operation'] == 'downloadfile') {
    ...
    if (!file_exists(DOWNLOAD_STATE_FILENAME)) {
        touch(DOWNLOAD_STATE_FILENAME);
    }
    file_put_contents(DOWNLOAD_STATE_FILENAME, '');
...
if (isset($_GET['operation']) && $_GET['operation'] == 'get_progress') {
    $rawData = file_get_contents(DOWNLOAD_STATE_FILENAME);
    echo $rawData;
